...
if __name__ == '__main__':
    ...
    # create figure & axes
    fig, axes = plt.subplots(len(weight_decays), len(data_types), figsize=(10,12))
    ...
    for fasta_file in fasta_files:
        ...
        for model_class_index, model_class in enumerate(model_classes):
            for weight_decay_ind, weight_decay in enumerate(weight_decays):

                # set model filename
                model_filename = fasta_file + ".vae.latentdim_5.weightdecay_{}.numhiddenunits_40.pt".format(weight_decay)
                ...
                for data_type_index, data_type in enumerate(data_types):
                    ...
                    for dist_pair in dist_pairs:
                        ...
                        ax = axes[weight_decay_ind, data_type_index]
    ...
    for weight_decay_ind in range(len(weight_decays)):
        for data_type_ind in range(len(data_types)):
            ax = axes[weight_decay_ind, data_type_ind]
            ax.set_yscale("log")
            ...
            if data_type_ind == 0:
                ...
                ax.set_yticklabels([])
            if data_type_ind == len(data_types)-1:
                ax.set_ylabel("W. Decay\n{}".format(weight_decays[weight_decay_ind]), rotation=0, labelpad=40)
...
fig.savefig("line_plot_mse_vae.pdf", bbox_extra_artists=(line_legend, marker_legend), bbox_inches='tight')
fig_scatter.savefig("scatter.pdf")
